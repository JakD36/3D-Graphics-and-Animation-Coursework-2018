...
void Scene1::Update(double currentTime) noexcept{
    ...
    m_lights[0].position = glm::vec3(0.0f,2.49f,0.0f) + glm::quat(glm::vec3(glm::radians(m_lightPitch),glm::radians(m_lightYaw),0.0f)) * glm::vec3(0.0f,0.0f,m_lightRadius);//+ Utils::Spherical2Cartesian(m_lightRadius, m_lightYaw, m_lightPitch); // here we set the position of light source
...
void Scene1::Turn(GLfloat yaw, GLfloat pitch) noexcept{
    ...
    m_lights[2].direction = glm::quat(glm::vec3(glm::radians(pitch),glm::radians(yaw),0.0f)) * glm::vec3(0.0f,0.0f,1.0f);//Utils::Spherical2Cartesian(1, yaw, pitch); // the light from the torch just goes where we are looking
}
